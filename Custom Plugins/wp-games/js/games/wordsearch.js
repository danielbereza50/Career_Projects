...
// https://codepen.io/minhein/pen/oOEBLo	
...
class App {
  constructor(div) {
    ...
    this.ctx = this.canvas.getContext('2d'); // Use this.canvas instead of canvas
    this.dirty = true;
    this.prev = +new Date();
   ...
   // console.log("Previous timestamp:", this.prev);
  ...
  draw() {
    ...
    this.ctx.fillStyle = "blue"; // Example fill color
    this.ctx.fill();
  }
...
class PuzzleApp extends App {
  constructor(div, puzzle) {
    super(div);
    this.puzzle = puzzle;
    this.renderList(document.getElementById("wordsearch-list"));
  ...
  touchup(x, y) {
    ...
    console.log(word);
    if (word) {
      let list = document.getElementById("wordsearch-list");
    ...
    let app = null;
    const currentPageId = $('#wordsearch-container').data('page-id');
    console.log(currentPageId);
		...
		jQuery.ajax({
			...
			type: 'GET',
			data: {
				action: 'get_wordsearch_data',
				currentPageId: currentPageId,
			},
			success: function(data) {
				try {
					// Parse JSON data if needed (assuming data is JSON)
					const wordBank = typeof data === 'object' ? data : JSON.parse(data);

					// Validate wordBank is an array
					if (!Array.isArray(wordBank)) {
						throw new Error("Expected an array of words");
					}

					// Extract words from wordBank
					const wordLists = wordBank.map(obj => obj.word);

					// Ensure at most 10 words are selected
					let selectedWordLists = [];
					//7
					for (let i = 0; i < wordLists.length; i++) {
						selectedWordLists.push(wordLists[i]);
					}

					// Create puzzle and app inside the success callback
					...
					// Create puzzle using the selected wordLists and directions
					let puzzle = new Puzzle(16, 16, selectedWordLists, directions);
					puzzle.print(); // Assuming puzzle.print() prints the puzzle


					// Initialize app with puzzle in the PuzzleApp container
					app = new PuzzleApp(document.getElementById("wordsearch-container"), puzzle);
				} catch (error) {
					console.error("Error processing word search data:", error);
